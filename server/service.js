...
module.exports = (config) => {
  ...
  service.get('/service/:location', (req, res) => {
    ...
    request.get('https://maps.googleapis.com/maps/api/geocode/json?address=' + req.params.location + '&key=' + config.google_geo_api_key, (err, response) => {
      ...
      request.get('https://maps.googleapis.com/maps/api/timezone/json?location=' + location.lat + ',' + location.lng + '&timestamp=' + timestamp + '&key=' + config.google_time_api_key, (err, response) => {
        if (err) {
          log.error(err);
          return res.sendStatus(500);
        }
